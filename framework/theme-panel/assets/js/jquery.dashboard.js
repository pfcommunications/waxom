...
//				alert('.fold.fold-'+current+'.fold-'+$(this).val());
		...
		if($('select[name="customize_enable"]').length > 0) {
		
			$('select[name="customize_enable"]').on('change', function(){

				if($(this).val() == 'yes') {
					$('.pagetitle-customize .hidden').show();
				} else {
					$('.pagetitle-customize .hidden').hide();
				}
			});
		
		}
		
		$('#waxom_options-predefined_colors input').bind('click', function() {
			...
			$('input#accent_color-color').attr('value', accent1).change();
			$('input#accent_color2-color').attr('value', accent2).change();
			$('input#accent_color3-color').attr('value', accent3).change();
			$('input#accent_color4-color').attr('value', accent4).change();
		});
...
//	});
	...
	jQuery('#mymetabox_revslider_0,#vc_teaser,#vntd_page_settings_advanced').addClass('closed');
