...
// Fix for the GT3 page builder: http://www.gt3themes.com/wordpress-gt3-page-builder-plugin/
    ...
    if ( ! class_exists( 'ReduxFrameworkInstances' ) ) {
        // Instance Container
        require_once get_template_directory() . '/framework/theme-panel/ReduxCore/inc/class.redux_instances.php';
        require_once get_template_directory() . '/framework/theme-panel/ReduxCore/inc/lib.redux_instances.php';
    }
...
// Don't duplicate me!
    if ( ! class_exists( 'ReduxFramework' ) ) {
        ...
        // Redux CDN class
        require_once get_template_directory() . '/framework/theme-panel/ReduxCore/inc/class.redux_cdn.php';

        // Redux API class  :)
        require_once get_template_directory() . '/framework/theme-panel/ReduxCore/inc/class.redux_api.php';

        // General helper functions
        require_once get_template_directory() . '/framework/theme-panel/ReduxCore/inc/class.redux_helpers.php';

        // General functions
        require_once get_template_directory() . '/framework/theme-panel/ReduxCore/inc/class.redux_functions.php';
        require_once get_template_directory() . '/framework/theme-panel/ReduxCore/inc/class.p.php';

        require_once get_template_directory() . '/framework/theme-panel/ReduxCore/inc/class.redux_filesystem.php';

        require_once get_template_directory() . '/framework/theme-panel/ReduxCore/inc/class.redux_admin_notices.php';

        // Welcome
        require_once get_template_directory() . '/framework/theme-panel/ReduxCore/inc/welcome/welcome.php';
        ...
        class ReduxFramework {
            ...
            public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
                ...
                if ( empty ( $this->args['footer_credit'] ) ) {
                    $this->args['footer_credit'] = '<span id="footer-thankyou">' . sprintf( __( 'Options panel created using %1$s', 'waxom' ), '<a href="' . esc_url( $this->framework_url ) . '" target="_blank">' . __( 'Redux Framework', 'waxom' ) . '</a> v' . self::$_version ) . '</span>';
                }

                if ( empty ( $this->args['menu_title'] ) ) {
                    $this->args['menu_title'] = __( 'Options', 'waxom' );
                }

                if ( empty ( $this->args['page_title'] ) ) {
                    $this->args['page_title'] = __( 'Options', 'waxom' );
                }
...
// __construct()
            ...
            private function _internationalization() {
                 ...
                 * @param string     'waxom'  text domain
                 */
                $locale = apply_filters( "redux/textdomain/{$this->args['opt_name']}", get_locale(), 'waxom' );
                ...
                if ( ReduxFramework::$_is_plugin ) {
                    load_plugin_textdomain( 'waxom', '', self::$_dir . 'languages/' . $locale . '.mo' );
                } else {
                    load_textdomain( 'waxom', self::$_dir . 'languages/' . $locale . '.mo' );
                }
...
// get_instance()
            ...
            private function _tracking() {
                if ( file_exists( get_template_directory() . '/framework/theme-panel/ReduxCore/inc/tracking.php' ) ) {
                    require_once get_template_directory() . '/framework/theme-panel/ReduxCore/inc/tracking.php';
...
// show()
            ...
            private function _default_cleanup() {
                ...
                if ( Redux_Helpers::isLocalHost() && $false == true || ( Redux_Helpers::isWpDebug() )  && $false == true  ) {
            ...
            private function add_submenu( $page_parent, $page_title, $menu_title, $page_permissions, $page_slug ) {
                ...
                if ( isset ( $test[ $page_parent ] ) ) {
                    ...
                    if ( true == $addMenu ) {
                        ...
                        $this->page = call_user_func( 'add_' . 'submenu_page', $page_parent, $page_title, $menu_title, $page_permissions, $page_slug, array(
                            &$this,
                            'generate_panel'
                        ) );
                    }
            ...
            public function _options_page() {
                ...
                if ( $this->args['menu_type'] == 'hidden' ) {
                    ...
                    $this->page = call_user_func( 'add_' . 'menu_page', $this->args['page_title'], $this->args['menu_title'], $this->args['page_permissions'], $this->args['page_slug'], array(
                        &$this,
                        'generate_panel'
                    ...
                    if ( true === $this->args['allow_sub_menu'] ) {
                        if ( ! isset ( $section['type'] ) || $section['type'] != 'divide' ) {
                            foreach ( $this->sections as $k => $section ) {
                                ...
                                // ONLY for non-wp.org themes OR plugins. Theme-Check alert shown if used and IS theme.
                                call_user_func( 'add_' . 'submenu_page', $this->args['page_slug'], $section['title'], $section['title'], $this->args['page_permissions'], $this->args['page_slug'] . '&tab=' . $k,
...
// _enqueue()
            ...
            public function _load_page() {
                ...
                if ( true == $this->show_hints ) {
                    ...
                    $event = __( 'moving the mouse over', 'waxom' );
                    if ( 'click' == $this->args['hints']['tip_effect']['show']['event'] ) {
                        $event = __( 'clicking', 'waxom' );
                    }

                    // Construct message
                    $msg = sprintf( __( 'Hints are tooltips that popup when %d the hint icon, offering addition information about the field in which they appear.  They can be %d d by using the link below.', 'waxom' ), $event, strtolower( $s ) ) . '<br/><br/>' . $url;

                    // Construct hint tab
                    $tab = array(
                        'id'      => 'redux-hint-tab',
                        'title'   => __( 'Hints', 'waxom' ),
                        'content' => '<p>' . $msg . '</p>'
                    );
...
// admin_footer_text()
            ...
            private function get_default_output_string( $field ) {
                ...
                if ( ! empty ( $default_output ) ) {
                    $default_output = __( 'Default', 'waxom' ) . ": " . substr( $default_output, 0, - 2 );
                }
...
// _register_settings()
            ...
            private function _register_extensions() {
                $path    = get_template_directory() . '/framework/theme-panel/ReduxCore/inc/extensions/';
            ...
            public function ajax_save() {
                if ( ! wp_verify_nonce( $_REQUEST['nonce'], "redux_ajax_nonce" . $this->args['opt_name'] ) ) {
                    echo json_encode( array(
                        'status' => __( 'Invalid security credential.  Please reload the page and try again.', 'waxom' ),
                        'action' => ''
                    ) );
                ...
                if ( ! current_user_can( $this->args['page_permissions'] ) ) {
                    echo json_encode( array(
                        'status' => __( 'Invalid user capability.  Please reload the page and try again.', 'waxom' ),
                        'action' => ''
                    ) );
                ...
                if ( ! empty ( $_POST['data'] ) && ! empty ( $redux->args['opt_name'] ) ) {
                    ...
                    if ( ! empty ( $values ) ) {
                        ...
                        echo json_encode( array( 'status' => __( 'Your panel has no fields. Nothing to save.', 'waxom' ) ) );
                    }
                }
                if ( isset ( $this->transients['run_compiler'] ) && $this->transients['run_compiler'] ) {
                            ...
                            if (is_array($arr) && !empty($arr)) {
                                foreach($arr as $x => $y) {
                                    if (strpos(strtolower($y), 'redux') >= 0) {
                                        $msg = __('<strong>Redux Framework Notice: </strong>There are references to the Redux Framework support site in your config\'s <code>admin_bar_links</code> argument.  This is sample data.  Please change or remove this data before shipping your product.', 'waxom');
                    ...
                    if (!empty($this->args['share_icons'])) {
                        foreach($this->args['share_icons'] as $idx => $arr) {
                            if (is_array($arr) && !empty($arr)) {
                                foreach($arr as $x => $y) {
                                    if (strpos(strtolower($y), 'redux') >= 0) {
                                        $msg = __('<strong>Redux Framework Notice: </strong>There are references to the Redux Framework support site in your config\'s <code>share_icons</code> argument.  This is sample data.  Please change or remove this data before shipping your product.', 'waxom');
