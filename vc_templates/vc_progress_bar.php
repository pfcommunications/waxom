...
$class_to_filter = 'vc_progress_bar progress-bar-'.esc_attr( $style ).' wpb_content_element';
...
foreach ( $graph_lines_data as $line ) {
	...
	$output .= '<div class="vc_general vc_single_bar' . ( ( isset( $line['color'] ) && 'custom' !== $line['color'] ) ?
		...
		. '">';
	$output .= '<small class="vc_label"' . $line['txtcolor'] . '><span class="progress-bar-value">' . $line['label'] . $unit . '</span></small>';
	if ( $max_value > 100.00 ) {
