...
function waxom_woocommerce_init_settings()
{
	$catalog = array(
		'width' 	=> '300',	// px
		'height'	=> '300',	// px
	...
	wp_register_style('vntd-woocommerce-custom', get_template_directory_uri() . '/woocommerce/assets/woocommerce-styling.css', array('woocommerce-general', 'woocommerce-layout'));
	
	wp_enqueue_style('vntd-woocommerce-custom');
}

add_action('init', 'waxom_woocommerce_init_settings', 1);

add_filter('loop_shop_columns', 'waxom_loop_columns');
if (!function_exists('waxom_loop_columns')) {
	function waxom_loop_columns() {
...
add_action('woocommerce_before_shop_loop_item_title', 'waxom_woocommerce_loop_thumbnail', 10);
...
function waxom_product_categories() {
	global $post;
...
add_action('woocommerce_after_shop_loop_item_title', 'waxom_woocommerce_product_details', 10);
...
add_action('woocommerce_shop_loop_item_title', 'waxom_woocommerce_shop_loop_item_title');

if(!function_exists('waxom_woocommerce_shop_loop_item_title')) {
	function waxom_woocommerce_shop_loop_item_title() {
...
if(!function_exists('waxom_woo_nav_cart')) {
	function waxom_woo_nav_cart() {
		global $woocommerce;
...
add_filter( 'woocommerce_output_related_products_args', 'waxom_jk_related_products_args' );

if(!function_exists('waxom_jk_related_products_args')) {
	function waxom_jk_related_products_args( $args ) {
...
add_action('woocommerce_before_shop_loop', 'waxom_woocommerce_filters', 10);

if(!function_exists('waxom_woocommerce_filters')) {
	function waxom_woocommerce_filters() {
		...
		if(!$orderby) $orderby = esc_html__('Default Order','waxom');
		//
		
		$output = '<div id="vntd-woocommerce-heading">';
		
		$output .= '<ul id="vntd-woocommerce-filters" class="vntd-woocommerce-filters product-filters"><li class="product-orderby"><span>'.esc_html__('Sort by','waxom').' <strong>'.ucfirst($orderby).'</strong><i class="fa fa-angle-down"></i></span><ul>';
	
		// OrderBy	
		
		$orderby_list = array('default' => esc_html__('Default Order','waxom'),'title' => esc_html__('Name','waxom'),'price' => esc_html__('Price','waxom'),'date' => esc_html__('Date','waxom'),'popularity' => esc_html__('Popularity','waxom'));
		...
		foreach($orderby_list as $single_order) {
			...
			$output .= '<li><a href="?'.$params_orderby_url.'">'.esc_html__('Sort by','waxom').' '.$single_order.'</a></li>';
		}
		...
		$output .= '<li class="product-count"><span>'.esc_html__('Show','waxom').' <strong>'.$current_count.' '.esc_html__('products','waxom').'</strong><i class="fa fa-angle-down"></i></span><ul>';
		...
		foreach ($count_array as $count) {
			...
			$output .= '<li><a href="?'.http_build_query($params_count).'">'.esc_html__('Show','waxom').' <span>'.$count.' '.esc_html__('Products','waxom').'</span></a></li>';
		}
...
add_action('woocommerce_get_catalog_ordering_args', 'waxom_woocommerce_ordering', 20);

if(!function_exists('waxom_woocommerce_ordering')) {
	function waxom_woocommerce_ordering($args) {
...
if($product_count) {
	add_filter( 'loop_shop_per_page', function( $cols ) use ( $product_count ) { return (int) $product_count; }, 20 );
}
